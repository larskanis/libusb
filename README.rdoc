...
In order to use LIBUSB, you need the libusb-1.0 library (but not necessarily its header files).
* On a Debian or Ubuntu system, install the +libusb-1.0-0+ package.
* The gem for Windows comes with a precompiled +libusb.dll+, so there is no need for a compiler.
* On OS-X: One easy way to do this is with homebrew: +brew install libusb+
