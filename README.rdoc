...
{LIBUSB::Context#devices} is used to get all or only particular devices. A {LIBUSB::Device}
can be used to communicate with the USB device by {LIBUSB::DevHandle#control_transfer}, {LIBUSB::DevHandle#bulk_transfer} or {LIBUSB::DevHandle#interrupt_transfer}.
...
LIBUSB-gem for Windows comes with a precompiled _libusb.dll_ and can be used together with a precompiled FFI-gem, so there should be no need for a compiler.
